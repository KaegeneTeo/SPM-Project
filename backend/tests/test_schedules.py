...
def test_get_own_schedule(schedules_service, supabase_mock):
    ...
    mock_response.data = [{'Staff_ID': staff_id, 'schedule': [{'schedule_id': 1, 'date': '2024-10-16', 'time_slot': 1}]}]
    ...
    supabase_mock.from_().select.assert_called_once_with('Staff_ID, Staff_FName, Staff_LName, Dept, schedule!inner(schedule_id, staff_id, date, time_slot)')
...
def test_get_all_employees(schedules_service, supabase_mock):
    mock_response = MagicMock()
    mock_response.data = [{'Staff_ID': 1, 'Staff_FName': 'John', 'Staff_LName': 'Doe'}]
    ...
    assert result.data == mock_response.data
    supabase_mock.from_().select.assert_called_once_with('Staff_ID, Staff_FName, Staff_LName')

# Test case for getting schedules for all departments
def test_get_schedules_for_all_depts(schedules_service, supabase_mock):
    mock_response = MagicMock()
    mock_response.data = [{'Staff_ID': 1, 'Dept': 'HR', 'schedule': [{'schedule_id': 1, 'date': '2024-10-16', 'time_slot': 1}]}]
...
def test_get_schedules_by_dept(schedules_service, supabase_mock):
    dept = "HR"
    mock_response = MagicMock()
    mock_response.data = [{'Staff_ID': 1, 'Dept': dept, 'schedule': [{'schedule_id': 1, 'date': '2024-10-16', 'time_slot': 1}]}]
...
def test_get_schedules_by_reporting_manager(schedules_service, supabase_mock):
    ...
    reporting_manager = 1
    mock_response = MagicMock()
    mock_response.data = [{'Staff_ID': 1, 'Dept': dept, 'Reporting_Manager': reporting_manager, 'schedule': [{'schedule_id': 1, 'date': '2024-10-16', 'time_slot': 1}]}]
    ...
    select_mock.eq.assert_any_call("Reporting_Manager", reporting_manager)
...
def test_get_all_directors(schedules_service, supabase_mock):
    ...
    assert result.data == mock_response.data
    supabase_mock.from_().select.assert_called_once_with('Staff_ID, Staff_FName, Staff_LName')
    select_mock.eq.assert_called_once_with("Reporting_Manager", reporting_manager)
    select_mock.neq.assert_called_once_with("Postion", "MD")
...
def test_get_directors_schedules(schedules_service, supabase_mock):
    ...
    assert result.data == mock_response.data
    supabase_mock.from_().select.assert_called_once_with('Staff_ID, Staff_FName, Staff_LName, schedule!inner(schedule_id, staff_id, date, time_slot)')
    select_mock.eq.assert_called_once_with("Reporting_Manager", reporting_manager)
    select_mock.neq.assert_called_once_with("Postion", "MD")
...
def test_format_schedules(schedules_service):
    response = MagicMock()
    response.data = [{'Staff_ID': 1, 'Dept': 'HR', 'Staff_FName': 'John', 'Staff_LName': 'Doe', 'schedule': [{'schedule_id': 1, 'date': '2024-10-16', 'time_slot': 1}]}]
    
    allnames = MagicMock()
    allnames.data = [{'Staff_ID': 1, 'Staff_FName': 'John', 'Staff_LName': 'Doe'}]
    
    result, status_code = schedules_service.format_schedules(response, allnames)
    assert result == {'schedules': [{'start': '2024-10-16 09:00', 'end': '2024-10-16 13:00', 'class': 'AM', 'WFH': ['1 - John Doe'], 'count': 1, 'title': 1, 'inOffice': []}]}
